...
/** Strip style: 1 = alternating police lights (red/blue), 0 = mode colors */
#define POLICE_STRIP_LIGHTS 1
#define POLICE_BLINK_INTERVAL_MS 180
...
bool rgbStripEnabled = true;
uint32_t policeBlinkTick = 0;
bool policeBlinkBlue = false;

/* telemetry cache to keep onReceive() lightweight */
float telemetryBatteryVoltage = 0.0;
float telemetryBatteryRawVoltage = 0.0;
uint8_t telemetryBatteryPercentage = 0;
int telemetryIrLeftBlocked = 0;
int telemetryIrRightBlocked = 0;
float telemetryUsDistance = -1.0;
uint32_t telemetryBatteryTick = 0;
uint32_t telemetryIrTick = 0;
uint32_t telemetryUsTick = 0;

const uint16_t TELEMETRY_BATTERY_INTERVAL_MS = 400;
const uint16_t TELEMETRY_IR_INTERVAL_MS = 100;
const uint16_t TELEMETRY_US_INTERVAL_MS = 220;
//@}

void updateTelemetryCache() {
  uint32_t now = millis();

  if (now - telemetryBatteryTick >= TELEMETRY_BATTERY_INTERVAL_MS) {
    telemetryBatteryRawVoltage = batteryGetVoltage();
    telemetryBatteryPercentage = batteryGetPercentage();
    telemetryBatteryVoltage = batteryGetDashboardVoltage();
    telemetryBatteryTick = now;
  }

  if (now - telemetryIrTick >= TELEMETRY_IR_INTERVAL_MS) {
    byte result = irObstacleRead();
    telemetryIrLeftBlocked = int(!bool(result & 0b00000010));
    telemetryIrRightBlocked = int(!bool(result & 0b00000001));
    telemetryIrTick = now;
  }

  // Ultrasonic can block up to timeout, so sample less frequently.
  if (now - telemetryUsTick >= TELEMETRY_US_INTERVAL_MS) {
    telemetryUsDistance = int(ultrasonicRead() * 100) / 100.0;
    telemetryUsTick = now;
  }
}

void updateStripByModeColor(uint32_t modeColor) {
  if (!rgbStripEnabled) {
    rgbOff();
    return;
  }

#if POLICE_STRIP_LIGHTS
  uint32_t now = millis();
  if (now - policeBlinkTick >= POLICE_BLINK_INTERVAL_MS) {
    policeBlinkTick = now;
    policeBlinkBlue = !policeBlinkBlue;
  }
  rgbWrite(policeBlinkBlue ? BLUE : RED);
#else
  rgbWrite(modeColor);
#endif
}
...
void loop() {
#if !TEST
  bool wasDisconnected = (currentMode == MODE_DISCONNECT);
...
void modeHandler() {
  switch (currentMode) {
    case MODE_NONE:
      updateStripByModeColor(MODE_NONE_COLOR);
      carStop();
      servo.write(servoAngle);
      break;
    case MODE_DISCONNECT:
      updateStripByModeColor(MODE_DISCONNECT_COLOR);
      carStop();
      servo.write(servoAngle);
      break;
    case MODE_OBSTACLE_FOLLOWING:
      updateStripByModeColor(MODE_OBSTACLE_FOLLOWING_COLOR);
      servo.write(servoAngle);
      obstacleFollowing();
      break;
    case MODE_OBSTACLE_AVOIDANCE:
      updateStripByModeColor(MODE_OBSTACLE_AVOIDANCE_COLOR);
      servo.write(servoAngle);
      obstacleAvoidance();
      break;
    case MODE_APP_CONTROL:
      updateStripByModeColor(MODE_APP_CONTROL_COLOR);
      servo.write(servoAngle);
      carSetMotors(leftMotorPower, rightMotorPower);
      break;
    case MODE_VOICE_CONTROL:
      updateStripByModeColor(MODE_VOICE_CONTROL_COLOR);
      servo.write(servoAngle);
      voice_control();
      break;
    default:
...
void obstacleFollowing() {
  ...
  float usDistance = telemetryUsDistance;
...
void obstacleAvoidance() {
  ...
  bool middleIsClear = (telemetryUsDistance < 0 || telemetryUsDistance > ULTRASONIC_AVOIDANCE_THRESHOLD);
...
void onReceive() {
  // --------------------- send data ---------------------
  // battery voltage
  aiCam.sendDoc["BV"] = telemetryBatteryVoltage;
  aiCam.sendDoc["BP"] = telemetryBatteryPercentage;
  aiCam.sendDoc["BR"] = telemetryBatteryRawVoltage;

  // IR obstacle detection data
  aiCam.sendDoc["N"] = telemetryIrLeftBlocked;   // left, clear:0
  aiCam.sendDoc["P"] = telemetryIrRightBlocked;  // right, clear:0

  // ultrasonic
  aiCam.sendDoc["O"] = telemetryUsDistance;

  // GPS
  if (gpsHasTelemetryUpdate()) {
    aiCam.sendDoc["GFX"] = int(gpsHasFix());
    aiCam.sendDoc["GLA"] = gpsHasFix() ? gpsGetLatitude() : 0.0;
    aiCam.sendDoc["GLO"] = gpsHasFix() ? gpsGetLongitude() : 0.0;
    aiCam.sendDoc["GSP"] = gpsGetSpeedKmph();
    aiCam.sendDoc["GSA"] = gpsGetSatellites();
    gpsMarkTelemetrySent();
  }
  ...
  // light controls: strip (L) and camera lamp (M)
  rgbStripEnabled = aiCam.getSwitch(REGION_L);
